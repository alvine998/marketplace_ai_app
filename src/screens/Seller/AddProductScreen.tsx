...
const AddProductScreen = ({ navigation }: any) => {
    ...
    const validate = () => {
        if (!name || !description || !price || !stock || !selectedCategory) {
            Alert.alert('Error', 'Mohon lengkapi semua field wajib (*) sentimen.');
            return false;
        }
        if (!image) {
            Alert.alert('Error', 'Mohon pilih gambar produk.');
            return false;
        }
        if (isFlashSale) {
            if (!flashSalePrice) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (response && (response.status === 201 || response.status === 200 || !response.error)) {
                Toast.show({
                    type: 'success',
                    text1: 'Sukses',
                ...
                Toast.show({
                    type: 'error',
                    text1: 'Gagal',
                    text2: response?.message || 'Gagal menambahkan produk',
                });
            }
            ...
            <ScrollView contentContainerStyle={styles.content}>
                ...
                <TouchableOpacity style={styles.imagePicker} onPress={handleSelectImage}>
                    {image ? (
                        ...
                        <View style={styles.uploadPlaceholder}>
                            ...
                            <Text style={styles.uploadText}>Tambah Foto Produk *</Text>
                        </View>
                    )}
                </TouchableOpacity>
                ...
                >
                    {loading ? (
                        ...
                        <Text style={styles.submitBtnText}>Simpan Produk</Text>
                    )}
                </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    input: {
        ...
        borderRadius: SIZES.radius,
        paddingHorizontal: SPACING.md,
        paddingVertical: Platform.OS === 'ios' ? SPACING.md : SPACING.sm, // detailed adjust
        fontSize: normalize(14),
        color: COLORS.black,
